...
+
  ...
  const providerHtml='<div class="ci-section"><h4>Analysis providers</h4>'+(providers.length?providers.map(function(p){return '<p class="ci-narrative"><strong>'+esc(p.label)+':</strong> '+esc(p.statusLabel)+(p.capturedAt?' · '+fmt(p.capturedAt):'')+' · '+esc(p.source||'')+'</p>';}).join(''):'<p class="ci-narrative">Provider status unavailable.</p>')+'<p class="ci-narrative" style="font-size:.72rem;color:#94a3b8">Opening this dashboard does not start analysis. Generate uses the existing orchestrate_competitor_analysis job, which runs the configured competitor-analysis pipeline (Google Places local discovery and the configured DataForSEO organic-search provider where that artifact exists).</p></div>';
  ...
  const organicHtml='<div class="ci-section"><h4>DataForSEO organic-search competitors</h4>'+
    '<p class="ci-narrative"><strong>Analysis status:</strong> '+(organic.generated?'Generated':'Not generated')+(organic.capturedAt?' · '+fmt(organic.capturedAt):'')+' · '+esc(organic.provider||'dataforseo-google-organic-live')+'</p>'+
    (organic.generated?(organicRows.length?'<table class="audit-table"><thead><tr><th>Competitor</th><th>Domain</th><th>URL</th><th>Evidence</th><th>Source</th></tr></thead><tbody>'+
    organicRows.map(c=>'<tr><td>'+esc(c.name||'')+'</td><td>'+esc(c.domain||'')+'</td><td>'+esc(c.url||'')+'</td><td>'+esc(c.evidence||'')+'</td><td>'+esc(c.source||'')+'</td></tr>').join('')+'</tbody></table>':'<p class="ci-narrative">DataForSEO organic-search analysis generated but no competitors returned.</p>'):
    '<p class="ci-narrative"><strong>DataForSEO organic-search competitors not yet generated</strong></p><p class="ci-narrative">This dashboard does not invent organic-search competitors. Generate Competitor Analysis uses the existing configured DataForSEO path; results appear here when that artifact exists.</p>'+(ca.generated?generateBtn:''))+'</div>';

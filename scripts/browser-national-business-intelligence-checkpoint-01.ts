 ...
 * Deterministic local render. Does not call paid APIs.
 */
...
async function main() {
  ...
  check("website-content", /What website content already exists/i.test(bi.text) && /Total pages discovered/i.test(bi.text), view.inventory.origin);
  ...
  check(
    ...
    /READY FOR COMPETITOR DISCOVERY/.test(bi.text) && new RegExp(`data-ready-for-competitor-discovery="${view.readyForCompetitorDiscovery ? "yes" : "no"}"`).test(bi.html),
  ...
  check("wi-inventory-contract", /Total pages discovered|NOT YET CONNECTED/i.test(wi.text), "inventory contract");
